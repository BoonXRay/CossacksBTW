...
static const int kZCacheMaxElements = 4096;//TODO: increase? disappearing trees etc. if buffer too small?
//beware: simple multiplying won't work - buildings will partly disappear at some point...

//Array of classes, 114688 bytes
static ZElement ZCache[kZCacheMaxElements];

//Current existing elements in ZCache (max value kZCacheMaxElements - 1)
static int NZElm;

static constexpr uint16_t NYLines = 2000;
static constexpr uint16_t DYZBuf = 256;
static constexpr uint8_t ZSHF  = 6;
static constexpr uint8_t ZBFNX = 1<<ZSHF;   //64

static uint8_t ZBFCounter[NYLines];
static word ZBUF[NYLines*ZBFNX];
static word LOLAYER[1024];
static int NLOITEMS;
static word HILAYER[1024];
static int NHIITEMS;
static short ZBUFX[NYLines*ZBFNX];
//debug only!
static word ZBUF_OLD[NYLines*ZBFNX];
static word ZBUFX_OLD[NYLines*ZBFNX];
...
static void SortWords( int NWords, word* Data, short* Factor )
{
    if (NWords < 2)
    {
        return;
...
static void PtLine( int x, int y, int x1, int y1, byte c )
{
...
static void ShowZElement( ZElement* ZEL )
{
    ZEL->IsDrawn = true;
