...
static uint8_t AssignTBL[8] = { 0,1,2,3,4,5,6,7 };

static char* CurAIDLL = NULL;
static void AIER( const char* Mess )
{
    MessageBox( NULL, Mess, CurAIDLL, MB_TOPMOST );
}

static void AI_Error()
{
    AIER( "ERROR: AI function must be called only from AI section." );
}

static void ScenErr( char* Mess )
{
...
static void ScenErr( const char * Mess, char* Par1 )
{
    char ccc[160];
...
void IntErr( const char* Mess, char* s1 )
{
	char ccc[256];
	sprintf( ccc, Mess, s1 );
	IntErr( ccc );
...
extern "C" __declspec( dllexport ) bool RegisterString( GAMEOBJ* GOBJ, char* ID )
{
    ...
    //GOBJ->Type = 'STRI';
    GOBJ->Type = ConvCosChar( 'S', 'T', 'R', 'I' );
	GOBJ->Index = SCENINF.NMess;
	GOBJ->Serial = 0;
	if ( SCENINF.NMess >= SCENINF.MaxMess )
	{
		SCENINF.MaxMess += 64;
        SCENINF.Messages = (const char**) realloc( SCENINF.Messages, SCENINF.MaxMess * 4 );
	}
...
extern "C" __declspec( dllexport ) void DisableUpgrade( byte Nat, GAMEOBJ* Upg )
{
	...
	{
        //if ( Upg->Type != 'UPGR' )
        if ( Upg->Type != ConvCosChar( 'U', 'P', 'G', 'R' ) )
		{
...
extern "C" __declspec( dllexport ) void EnableUnit( byte Nat, GAMEOBJ* Type, bool State )
{
	...
	{
        //if ( Type->Type != 'UTYP' )
        if ( Type->Type != ConvCosChar( 'U', 'T', 'Y', 'P' ) )
		{
...
extern "C" __declspec( dllexport ) int GetUnitsAmount0( GAMEOBJ* Zone, byte Nation )
{
    ...
    if ( ( Zone->Type & 0xFF000000 ) == ( ConvCosChar( '@', ' ', ' ', ' ' ) - 0x202020 ) )
	{
    ...
    if ( ( !Zone ) || Zone->Type != ConvCosChar( 'Z', 'O', 'N', 'E' ) )
	{
...
extern "C" __declspec( dllexport ) void ShowAlarm( GAMEOBJ* Zone )
{
	...
	{
        //if ( Zone->Type == 'ZONE' )
        if ( Zone->Type == ConvCosChar( 'Z', 'O', 'N', 'E' ) )
		{
...
{
    //if ( Units->Type != 'UNIT' )
    if ( Units->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )
	{
...
extern "C" __declspec( dllexport ) int GetUnitsAmount2( GAMEOBJ* Zone, GAMEOBJ* UnitType, byte Nation )
{
    ...
    if ( UnitType->Type != ConvCosChar( 'U', 'T', 'Y', 'P' ) )
	{
...
extern "C" __declspec( dllexport ) bool IsUpgradeDoing( GAMEOBJ* Upgrade, byte Nation )
{
    ...
    if ( Upgrade->Type != ConvCosChar( 'U', 'P', 'G', 'R' ) )
	{
...
extern "C" __declspec( dllexport ) int GetDied( GAMEOBJ* UnitType, byte Nation )
{
    ...
    if ( UnitType->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )
	{
...
extern "C" __declspec( dllexport ) bool CreateObject0( GAMEOBJ* DstObj, GAMEOBJ* Form, GAMEOBJ* UnitType, byte NatID, GAMEOBJ* Zone, byte Direction )
{
    ...
    //if ( Form->Type != 'FORM' )
    if ( Form->Type != ConvCosChar( 'F', 'O', 'R', 'M' ) )
	{
	...
	DstObj->Serial = 0;
    //DstObj->Type = 'UNIT';
    DstObj->Type = ConvCosChar( 'U', 'N', 'I', 'T' );
	return true;
}
//---------------------Action commands----------------------
...
extern "C" __declspec( dllexport ) void SelectUnitsType( GAMEOBJ* UnitsType, byte Nat, bool Add )
{
    ...
    if ( UnitsType->Type != ConvCosChar( 'U', 'T', 'Y', 'P' ) )
	{
...
extern "C" __declspec( dllexport ) void SelAttackGroup( byte Nat, GAMEOBJ* Enemy )
{
	Nat = AssignTBL[Nat];
    //if ( Enemy->Type != 'UNIT' )
    if ( Enemy->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )
	{
...
extern "C" __declspec( dllexport ) void AttackZoneByArtillery( GAMEOBJ* ArtGroup, GAMEOBJ* Zone, byte Nat )
{
    ...
    if ( ArtGroup->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )
	{
...
void SelChangeNation( uint8_t SrcNat, uint8_t DstNat )
{
...
{
    //if ( Hint->Type != 'STRI' )
    if ( Hint->Type != ConvCosChar( 'S', 'T', 'R', 'I' ) )
	{
    ...
    const char* cc = SCENINF.Messages[Hint->Index];
	if ( cc )
	{
...
void ShowCentralText( const char* ID, int time )
{
...
extern "C" __declspec( dllexport ) bool UnitsCenter( GAMEOBJ* DstZone, GAMEOBJ* Units, word R )
{
	...
	if ( N )
	{
        ...
        DstZone->Type = ConvCosChar( '@', ' ', ' ', ' ' ) - 0x202020 + R;
	}
	else
	{
		DstZone->Type = -1;
...
{
    //if ( Grp->Type != 'UNIT' )
    if ( Grp->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )
	{
...
extern "C" __declspec( dllexport ) int GetTopDst( GAMEOBJ* Z1, GAMEOBJ* Z2 )
{
    ...
    if ( ( Z1->Type & 0xFF000000 ) == ( ConvCosChar( '@', ' ', ' ', ' ' ) - 0x202020 ) )
	{
		xc1 = Z1->Index;
		yc1 = Z1->Serial;
	}
        ...
        //if ( Z1->Type != 'ZONE' )
        if ( Z1->Type != ConvCosChar( 'Z', 'O', 'N', 'E' ) )
		{
    ...
    if ( ( Z2->Type & 0xFF000000 ) == ( ConvCosChar( '@', ' ', ' ', ' ' ) - 0x202020 ) )
	{
		xc2 = Z2->Index;
		yc2 = Z2->Serial;
	}
        ...
        //if ( Z2->Type != 'ZONE' )
        if ( Z2->Type != ConvCosChar( 'Z', 'O', 'N', 'E' ) )
		{
...
static constexpr int DEVELOPMENT = 0x1001;
static constexpr int WATER_BATTLE = 0x1002;
static constexpr int LAND_BATTLE = 0x1003;
static constexpr int MINES_CPTURE = 0x1004;
static constexpr int TOWN_DEFENCE = 0x1005;
static constexpr int MINES_UPGRADE = 0x1006;
static constexpr int FAST_DIVERSION = 0x1007;
...
extern "C" __declspec( dllexport ) void RepairBuildingsBySel( byte Nat, GAMEOBJ* Buildings )
{
    ...
    if ( Buildings->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )
	{
...
extern "C" __declspec( dllexport ) void ProduceUnit( GAMEOBJ* Units, GAMEOBJ* UnitType, GAMEOBJ* DestGroup )
{
    ...
    if ( DestGroup->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )
	{
...
{
    //Units->Type = 'UNIT';
    Units->Type = ConvCosChar( 'U', 'N', 'I', 'T' );
	if ( SCENINF.NUGRP >= SCENINF.MaxUGRP )
	{
		SCENINF.MaxUGRP += 32;
        SCENINF.UGRP = (UnitsGroup*) realloc( SCENINF.UGRP, SCENINF.MaxUGRP * sizeof( UnitsGroup) );
...
{
    //if ( Source->Type != 'UNIT' )
    if ( Source->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )
	{
		IntErr( "RemoveGroup(Source,Dest) : Invalid <Source>" );
		return;
	}
    //if ( Dest->Type != 'UNIT' )
    if ( Dest->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )
	{
...
uint8_t CurAINation = 1;
City * CCIT = NULL;
Nation * CNAT;
// BoonXRay 17.08.2017
//#define PRC(x) ((32768*##x##)/100)
constexpr inline int PRC( int x ) noexcept { return ( 32768 * x )/100; }

const int PERCONV[101] = { PRC( 0 ),PRC( 1 ),PRC( 2 ),PRC( 3 ),PRC( 4 ),PRC( 5 ),PRC( 6 ),PRC( 7 ),PRC( 8 ),PRC( 9 ),
...
extern "C" __declspec( dllexport ) bool TryUpgrade( GAMEOBJ* Upgrade, int CostPercent, int Probability )
{
    ...
    if ( Upgrade->Type == ConvCosChar( 'S', 'A', 'F', 'E' ) )
	{
		return false;
	}
	if ( AiIsRunNow )
...
void SavePlayerData( const char* Name )
{
    char cc[200];
	...
	if ( F )
	{
		...
		for ( int i = 0; i < NCamp; i++ )
		{
			...
			if ( cc[0] && CampMessage[0] && CampText[0] && CampBmp[0] )
			{
                ...
                SCamp[i].OpenIndex = new uint32_t[SCamp[i].NMiss];
                // BoonXRay 03.09.2017
...
void SFLB_LoadPDATA( const char* Name )
{
	PLOAD = 0;
	char cc[200];
	sprintf( cc, "Players\\%s.txt", Name );
...
extern "C" __declspec( dllexport ) int InsertUnitToGroup( GAMEOBJ* Src, GAMEOBJ* Dst, int Index )
{
    ...
    if ( ( !Dst ) || ( Dst->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) ) )
	{
		return -1;
	}
	if ( Src )
	{
        //if ( !( Src->Type != 'UNIT' ) )
        if ( !( Src->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) ) )
		{
			return -1;
		}
...
extern "C" __declspec( dllexport ) void RemoveUnitFromGroup( GAMEOBJ* Src, int Index )
{
    ...
    if ( Src->Type != ConvCosChar( 'U', 'N', 'I', 'T' ) )return;
...
extern "C" __declspec( dllexport ) bool GetZoneCoor( GAMEOBJ* Zone, int* x, int* y )
{
    ...
    if ( ( Zone->Type == ConvCosChar( 'Z', 'O', 'N', 'E' ) ) && ( Zone->Index < SCENINF.NZGRP ) )
	{
		ActiveZone* AZ = AZones + SCENINF.ZGRP[Zone->Index].ZoneID[0];
		*x = AZ->x;
		*y = AZ->y;
