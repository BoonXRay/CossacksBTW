...
//------------class TextButton
int GetRLen( const char* s, RLCFont* font )
{
	if (!int( s ))return 0;
	int x = 0;
	int L;
...
GP_TextButton* DialogsSystem::addGP_TextButton(
    SimpleDialog* Parent, int x, int y, const  char* str,
...
UniversalButton* DialogsSystem::addUniversalButton(
    int x, int y, int Lx, const char* str, int GP_File,
...
bool LB_SORT_ITEM = 0;
void ListBox::AddItem( const char* str, int n )
{
	NeedToDraw = true;
...
void ListBox::AddStaticItem( const char* str, int n )
{
	NeedToDraw = true;
	ListBoxItem* LB = new ListBoxItem;
	if (LastItem)
...
bool InputBox_OnDraw( SimpleDialog* SD )
{
	...
	if (SD)
	{
        ...
        // BoonXRay 20.08.2017
        //strcpy( base, IB->Str );
        strncpy( base, IB->String.data(), 2048 );
		if (IB->Anonim)
		{
            // BoonXRay 20.08.2017
            //int L = strlen( IB->Str );
            int L = IB->String.length();
...
//		}
        if (IB->CursPos > IB->String.length())
            IB->CursPos = IB->String.length();
...
//			else xx1 += GetRLCWidth( IB->Font->RLC, IB->Str[j] );
            if (IB->Anonim)xx1 += GetRLCWidth( IB->Font->RLC, '*' );
            else xx1 += GetRLCWidth( IB->Font->RLC, IB->String[j] );
			if (xx1 >= xxx)
			{
                // BoonXRay 20.08.2017 These comparsions always true
//				if (j >= 0)IB->CursPos = j;
...
bool InputBox_OnKeyDown( SimpleDialog* SD )
{
	...
	if (LastKey == VK_LEFT)
	{
		...
		if (LastKey == VK_RIGHT)
		{
            ...
            if (IB->CursPos < IB->String.length())IB->CursPos++;
			KeyPressed = 0;
			LastKey = 0;
			return true;
		}
				...
				if (LastKey == VK_HOME)
				{
					...
					if (LastKey == VK_BACK)
					{
						if (IB->CursPos > 0)
						{
                            ...
                            IB->String.erase( IB->CursPos - 1, 1 );
							IB->CursPos--;
                        }
...
//							}
						...
						else
						{
							if (LastAsciiKey&&LastAsciiKey >= 32)
							{
                                ...
                                if ( IB->String.length() + 1 < DWORD( IB->StrMaxLen ))
								{
									char ccc[2048];
                                    //strcpy( ccc, IB->Str );
                                    strcpy( ccc, IB->String.data() );
									ccc[IB->CursPos] = 0;
									strcat( ccc, xx );
                                    //strcat( ccc, IB->Str + IB->CursPos );
                                    strcat( ccc, IB->String.data() + IB->CursPos );
                                    //strcpy( IB->Str, ccc );
                                    IB->String = ccc;
									IB->CursPos++;
                                }
...
void TextViewer::LoadFile( const char * TextFile )
{
	if (TextPtr)
	{
		free( TextPtr );
...
TextViewer* DialogsSystem::addTextViewer(
	SimpleDialog* Parent,
    int x, int y, int Lx, int Ly, const char* TextFile,
	RLCFont* Font )
{
	int i;
	for (i = 0; i < MAXDLG&&DSS[i] != nullptr; i++);
...
void ComboBox::AddLine( const char* Text )
{
	if (Lines)
	{
...
bool SafeLoad = 0;
void SQPicture::LoadPicture( const char* name )
{
	if (this->PicPtr)
	{
		free( PicPtr );
...
SQPicture::SQPicture( const char* Name )
{
	PicPtr = nullptr;
	LoadPicture( Name );
}
...
static bool MakeClipping(SimpleDialog* SD) {
    if (!SD->Enabled)return false;
    WindX = 0;
    WindY = 0;
    WindX1 = RealLx - 1;
