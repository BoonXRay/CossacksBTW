...
static void SAVMES( SaveBuf* ff1, const char* mes )
{
	char mss[16];
	memset( mss, 0, 16 );
	int len = strlen( mes );
...
static void SAVMES1( SaveBuf* ff1, const char* mes, int parm )
{
	char mss[16];
	char xxx[28];
...
static void LOADMES( SaveBuf* ff1 )
{
	char zzz[16];
	xBlockRead( ff1, zzz, 16 );
}

static void LOutErr( LPCSTR s )
{
...
static int DestnX[512];
static int DestnY[512];
static int NDestn;
...
//SAVING...
static void SaveRLE1( SaveBuf* SB, void* data, int Size )
{
	byte* Data = (byte*) data;
	byte bff[256];
	int dpos = 0;
...
static void LoadRLE1( SaveBuf* SB, void* data )
{
	byte* Data = (byte*) data;
	int Size;
...
static const int sfHeader = ConvCosChar('F', 'S', 'A', 'K');  // 'FSAK'
int sfVersion;

static void SaveMap( SaveBuf* SB )
{
...
static void LoadMap( SaveBuf* SB )
{
	xBlockRead( SB, &msx, 4 );
	xBlockRead( SB, &msy, 4 );
...
static void SaveNations( SaveBuf* SB )
{
...
static bool LoadNations( SaveBuf* SB )
{
	LoadAllNations( 0 );
	LoadAllNations( 1 );
	LoadAllNations( 2 );
...
static constexpr int OrdCount = 32;
static ReportFn* ORDLIST[OrdCount] = {
	&BuildObjLink,
	&ProduceObjLink,
	&AttackObjLink,
	&TakeResLink,
...
static ReportFn* GetOrderRef( int i )
{
	if (i == -1 || i >= OrdCount)return NULL;
	return ORDLIST[i];
}

static void SaveObjects( SaveBuf* SB )
{
    int tt = ConvCosChar('S', 'J', 'B', 'O');   // 'SJBO'
	xBlockWrite( SB, &tt, 4 );
	int NObjects = 0;
...
static void LoadObjects( SaveBuf* SB )
{
	int tt;
	xBlockRead( SB, &tt, 4 );
    if (tt != ConvCosChar('S', 'J', 'B', 'O'))  // 'SJBO'
        LOutErr( "Save file is corrupted." );;
	int NObjects = 0;
	LOADMES( SB );
...
static void SaveSelection( SaveBuf* SB )
{
    int i = ConvCosChar('S', 'E', 'L', 'I');    // 'SELI'
	xBlockWrite( SB, &i, 4 );
...
static void LoadSelection( SaveBuf* SB )
{
	int i;
	xBlockRead( SB, &i, 4 );
	LOADMES( SB );
    if (i != ConvCosChar('S', 'E', 'L', 'I'))   // 'SELI'
        LOutErr( "Save file is corrupted." );
	for (int i = 0; i < 80; i++)
	{
		SelGroup* SG = &SelSet[i];
...
static void SaveWalls( SaveBuf* SB )
{
    int i = ConvCosChar('L', 'L', 'A', 'W');    // 'LLAW'
	xBlockWrite( SB, &i, 4 );
	SAVMES( SB, " WALLSINFO" );
...
static void LoadWalls( SaveBuf* SB )
{
	int i;
	xBlockRead( SB, &i, 4 );
    if (i != ConvCosChar('L', 'L', 'A', 'W'))   // 'LLAW'
        LOutErr( "Save file is corrupted." );
	LOADMES( SB );
...
static void SaveAnmObj( SaveBuf* SB )
{
    int i = ConvCosChar('O', 'M', 'N', 'A');    // 'OMNA'
	xBlockWrite( SB, &i, 4 );
	int NExplosions = 0;
	...
	for (int i = 0; i < MaxExpl; i++)
	{
		if (EUsage[i])
		{
            ...
            // BoonXRay 26.08.2017 Now Sender is Index, later - Pointer by this Index ???
//			if (NAN.Sender)NAN.Sender = (OneObject*) ( NAN.Sender->Index );
//			else NAN.Sender = (OneObject*) 0xFFFFFFFF;
//			NAN.Weap = (Weapon*) NAN.Weap->MyIndex;
            if (NAN_.Sender)NAN_.Sender = reinterpret_cast<OneObject * >( NAN_.Sender->Index );
            else NAN_.Sender = reinterpret_cast<OneObject * >( -1 );
            NAN_.Weap = reinterpret_cast<Weapon * >( NAN_.Weap->MyIndex );
...
static void LoadAnmObj( SaveBuf* SB )
{
	CloseExplosions();
	InitExplosions();
	int i;
	xBlockRead( SB, &i, 4 );
    if (i != ConvCosChar('O', 'M', 'N', 'A'))   // 'OMNA'
        LOutErr( "Save file is corrupted." );
	int NExplosions;
...
static void SaveSprites( SaveBuf* SB )
{
    int i = ConvCosChar('I', 'R', 'P', 'S');    // 'IRPS'
	xBlockWrite( SB, &i, 4 );
	int NSprt = 0;
...
static void LoadSprites( SaveBuf* SB )
{
	int i;
	xBlockRead( SB, &i, 4 );
    if (i != ConvCosChar('I', 'R', 'P', 'S'))       // 'IRPS'
        LOutErr( "Save file is corrupted." );
...
static void Save3DBars( SaveBuf* SB )
{
    int i = ConvCosChar('R', 'A', 'B', '3');    // 'RAB3'
	xBlockWrite( SB, &i, 4 );
...
static void Load3DBars( SaveBuf* SB )
{
	int i;
	xBlockRead( SB, &i, 4 );
    if (i != ConvCosChar('R', 'A', 'B', '3'))   // 'RAB3'
        LOutErr( "Save file is corrupted." );
...
static void LS_SaveTopology( SaveBuf* SB )
{
...
static void LS_LoadTopology( SaveBuf* SB )
{
	ClearLinkInfo();
...
//--------------------------Saving AI-------------------------//
static constexpr int NLS_CFN = 6;
static CheckWorkFn* LS_CFN[NLS_CFN] = {
	&CheckBuilders,
	&CheckFree,
	&CheckFieldWorker,
	&CheckFreeArmy,
...
static constexpr int NBR_ORD = 14;
static BrigadeLink* BR_ORD[NBR_ORD] = {
	&B_LocalSendToLink,
...
static constexpr int NAR_ORD = 6;
static ArmyLink* AR_ORD[NAR_ORD] = {
	&ArmyLocalSendToLink,
...
static constexpr int N_IBR = 3;
static IdeaBrain* IBR[N_IBR] = {
	&TakeNewMineBrain,
	&UpgradeMineBrain,
	&BuildWallBrain
};

static void SaveAI( SaveBuf* SB )
{
			...
			for (int i = 0; i < MaxArm; i++)
			{
				if (CT->ARMS[i].Enabled)
				{
					...
					for (int j = 0; j < AR->NExBrigs; j++)
					{
                        ...
                        EBR.Brig = reinterpret_cast<Brigade* >( EBR.Brig->ID );
...
static const char* std_mess = "[?]";

static void LoadAI( SaveBuf* SB )
{
	int chk;
	xBlockRead( SB, &chk, 4 );
...
static void SaveActiveObjects( SaveBuf* f1 )
{
	int sz;
...
static void LoadActiveObjects( SaveBuf* f1 )
{
...
static void SaveMission( SaveBuf* SB )
{
...
static void LoadMission( SaveBuf* SB )
{
	int cms = -1;
...
static void SaveCost( SaveBuf* SB )
{
...
static void LoadCost( SaveBuf* SB )
{
	int NC = 0;
	xBlockRead( SB, &NC, 4 );
	NCost = NC;
...
static void PreSaveGame( SaveBuf* SB, const char* Messtr, int ID )
{
...
static int ARRSZ = 0;
static uint16_t * MRef = nullptr;//32768 bytes memset
static uint16_t* MCash = nullptr;
static int MaxMCash = 0;
static int CurMCash = 0;
static int ACTUAL_ADDSH;
...
void SelGroup::CreateFromSelection(uint8_t NI)
{
    if (int(Member))
    {
        DeleteMembers();
...
void SelGroup::SelectMembers(uint8_t NI, bool Shift)
{
    word MID;
    OneObject* OB;
    word Nsel = NSL[NI];
...
void SelGroup::ImSelectMembers(uint8_t NI, bool Shift) {
    word MID;
    OneObject* OB;
    word Nsel = ImNSL[NI];
    word* SMon = ImSelm[NI];
