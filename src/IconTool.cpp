...
void OneIcon::AssignIcon(uint16_t FID, uint16_t SID )
{
	Type = 0;
	Disabled = false;
	RPro = NULL;
...
void OneIcon::CreateHint( const char* str )
{
    // BoonXRay 19.08.2017
    if( str == NULL ) return;
    ...
    if (( !EditMapMode ) && (SpriteID != 0xFFFF) && KeyCodes[SpriteID][0])
	{
		strcat( Hint, " (" );
...
void OneIcon::AssignColor(uint8_t c )
{
	Type |= 1;
	Color = c;
}
...
void OneIcon::AssignLevel(uint8_t L )
{
	Level = L;
}

static void DrawInf( int x0, int y0, byte c )
{
...
OneIcon* IconSet::AddIconFixed(uint16_t FileID, uint16_t SpriteID, int Index )
{
	if (Index == -1)
	{
...
OneIcon* IconSet::AddIcon(uint16_t FileID, uint16_t SpriteID, const char* Hint )
{
...
OneIcon* IconSet::AddIcon(uint16_t FileID, uint16_t SpriteID, const char* Hint, uint8_t Color )
{
	OneIcon* OI = AddIcon( FileID, SpriteID, Hint );
	OI->AssignColor( Color );
	return OI;
}
OneIcon* IconSet::AddIcon(word FileID, word SpriteID, uint8_t Color, const char* Hint )
{
	OneIcon* OI = AddIcon( FileID, SpriteID, Hint );
	OI->AssignColor( Color );
	return OI;
...
void IconSet::DrawIconSet( int x, int y, int Nx, int Ny, int NyStart )
{
	...
	for (int i = 0; i < n; i++)
	{
		...
		if (OI->Visible)
		{
            ...
            if (DOKEYS&&(p != -1) && (OI->SpriteID != 0xFFFF) && KeyCodes[OI->SpriteID][0])
			{
