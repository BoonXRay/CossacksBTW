...
static char GetBestDirection(MotionField* MFI, int xd, int yd, int LX, char CurDir)
{
	int resdir = 0;
	int NDir = 0;
	int tdx = 0;
...
static void RetLock(OneObject* OB)
{
	int LX = OB->Lx;
...
static bool FindCostPosition(OneObject* OB, int* XDest, int* YDest, char* Direction)
{
	int LX = OB->Lx;
	MotionField* MFI = MFIELDS + OB->LockType;
...
static bool SearchTransportPlace(int* x, int* y, int Lx)
{
	int x0 = *x;
	int y0 = *y;
	for (int r = 0; r < 40; r++) {
...
static bool TRP_CheckDisconnectionAbility(OneObject* OB, GOrder* GOR, int LParam, int RParam)
{
	TransProcess* TRP = (TransProcess*)GOR->Data;
	return TRP->CheckDisconnectionAbility(OB, LParam, RParam);
}

static bool TRP_Disconnect(OneObject* OB, GOrder* GOR, int LParam, int RParam)
{
	TransProcess* TRP = (TransProcess*)GOR->Data;
...
static void AbsorbObject(OneObject* OB, OneObject* Absorber, byte OrdType)
{
...
static void LeaveShip(OneObject* OB, byte OrdType)
{
	if (OB->CheckOrderAbility())return;
...
static constexpr uint8_t MaxTP = 16;

static TransProcess TRAPR[MaxTP];
...
static constexpr uint8_t FishSX = 3;
uint8_t * FishMap;
int FishLx;
...
static bool GetMostSafePlace(int *x, int *y)
{
	int x0 = *x;
	int y0 = *y;
	int MIND = 100000000;
