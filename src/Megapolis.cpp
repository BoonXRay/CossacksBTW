...
void City::CreateCity( uint8_t N )
{
	...
	for ( int i = MinType; i <= MaxType; i++ )
	{
		...
		for ( int j = 0; j < PC; j++ )
		{
			...
			if ( NU->Individual || NU->StageUp )
			{
				...
				if ( NU->StageUp )
				{
                    uint8_t st = ( POB->StageState >> ( NU->Level * 3 ) ) & 7;
...
static int SearchPeasants( int x, int y, bool Workers, uint8_t NI, word* AddTo, int maxpos )
{
...
static int SearchPeasantsInSquare( int x, int y, int r, uint8_t NI, bool Workers, word* AddOn, int max )
{
	int pos = 0;
	for ( int i = 0; i <= r; i++ )
	{
...
static int CreateTeamOfPeasants( uint8_t NNUM, int x, int y, int MinP, int MaxP )
{
	word pids[128];
...
static constexpr int FieldR1 = 3;
...
int CURRENTAINATION;
static const uint8_t DefPreBDef[6] = { TowerID,5,6 };
static const uint8_t * PreBDef = DefPreBDef;
static int NPreBDef = 1;

static void SmartGamer( City* CT );
				...
				if ( !BPR.PlaceFound )
				{
					for ( int F = 0; F < 1 && PresentProject && !BPR.PlaceFound; F++ )
					{
						...
						if ( IDX >= 0 )
						{
							...
							{
                                uint8_t USAGE = IGO->newMons->Usage;
								bool used = 1;
...
void Branch::AddTo( uint8_t ResID, int Amount )
{
...
void Branch::AddPerc( uint8_t ResID, int Amount, int perc )
{
...
void Branch::AddEntire( uint8_t ResID, int Amount )
{
...
int Branch::GetMonsterCostPercent( uint8_t NI, word NIndex )
{
	int COST[8];
...
int Branch::GetUpgradeCostPercent( uint8_t NI, word NIndex )
{
...
void Nation::AddResource( uint8_t Rid, int Amount )
{
...
void Nation::ControlProduce( uint8_t Branch, uint8_t ResID, int Amount )
{
	switch ( Branch )
	{
	case 0://SELO
...
static const uint8_t BMIND[40] = { 6,6,6,6,6,6,6,6,1,3,
					  4,5,6,6,6,6,6,6,6,6,
					  1,2,1,0,2,6,6,6,6,6,
                      6,6,5,6,6,6,6,6,6,6 };
...
static int GetMaxCostPercent( uint8_t Nat, word* Cost )
{
	int maxp = 0;
	//int * res=RESRC[Nat];
	for ( int i = 0; i < 8; i++ )
...
static bool FindNearestWall( int* x, int* y, uint8_t NI )
{
...
int City::CheckDefending( uint8_t x, uint8_t y )
{
	for ( int i = 0; i < NDefn; i++ )
	{
...
void City::AddDefending( uint8_t x, uint8_t y, uint8_t Imp )
{
	if ( NDefn >= MaxDefn )
	{
		MaxDefn += 32;
...
static uint16_t BriMax[MaxBT] = { 256,256,256,256,256,36,256,16,256,256 };
static const uint16_t BriMin[MaxBT] = { 36 ,16 , 16, 16, 16, 1, 16,16, 25,  4 };
static const uint16_t DefBriMin[MaxBT] = { 4 ,16 , 16, 16, 16, 1, 16,16, 25,  4 };
...
static int CheckMinArmyCreationAbility( Brigade* BR )
{
	int N = BR->NMemb;
	word* Mem = BR->Memb;
	word NBMEM[MaxBT + 4];
...
static int CheckSuperMinArmyCreationAbility( Brigade* BR )
{
	int N = BR->NMemb;
	word* Mem = BR->Memb;
	word NBMEM[MaxBT + 4];
...
ArmyOrder* AI_Army::CreateOrder( uint8_t OrdType, int Size )
{
...
static __forceinline bool ValidTXY( int x, int y )
{
	int TX = TopLx - 1;
	return x > 0 && y > 0 && x < TX&&y < TX;
}

static void CheckTopPos( int xc, int yc, int* xd, int* yd )
{
	int x = *xd;
	int y = *yd;
	int mx = msx >> 1;
...
static const char* AO_LST = "[AI_Army::LocalSendTo]";
static bool CheckArmyDanger( AI_Army* ARM );
...
void AI_Army::LocalSendTo( int px, int py, uint8_t Prio, uint8_t OrdType )
{
	if ( !NExBrigs )
		return;
...
void AI_Army::WideLocalSendTo( int px, int py, uint8_t Prio, uint8_t OrdType )
{
	if ( !NExBrigs )
		return;
...
//++++++++++++++++++++++++++ CONNECT TO ARMY ++++++++++++++++++++++++//
//+++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++//
static const char* ACTA_Message = "[AI_Army::ConnectToArmy]";
...
void AI_Army::ConnectToArmy( int ID, uint8_t Prio, uint8_t OrdType )
{
...
static int GetTopDanger( int Top, int r, int NI )
{
...
static bool HandleArchers( AI_Army* ARM, int R );
...
static const char* ABTL_Mess = "[AI_Army::Make Battle]";
...
int  AI_Army::GetAmountOfBrigs( uint8_t Type )
{
	int N = 0;
	for ( int i = 0; i < NExBrigs; i++ )
	{
...
static void SHIP_DIVERSIA( AI_Army* ARM )
{
	rando();
...
static bool FastSearchGreAttObject( int* x, int* y, int r, uint8_t NI );
static void GrenaderSuperman( OneObject* OB );
...
//-------------------------------------Bitva-------------------------------------//
static const char* ABIT_Message = "[Army::Bitva]";
//#define maxen
class Army_Bitva :public ArmyOrder
{
public:
    uint8_t BitMask[1024];
	word Enm[512];
	word EnSN[512];
    uint8_t NDang[512];
	int  NEn;
	...
	int MaxX;
	int MaxY;
    void AddEnemXY( int x, int y, int MyTop, uint8_t msk );
};
...
void Army_Bitva::AddEnemXY( int x, int y, int MyTop, uint8_t msk )
{
...
static int FindStrongestUnitInCell( int cell, int* MinStrength, uint8_t NAT )
{
	cell += VAL_MAXCX + 1;
...
static int FindStrongestUnit( int x, int y, uint8_t NAT )
{
...
static const char* ADIV_Message = "[AI_Army::Diversion]";

class DiversionOrder :public ArmyOrder
{
public:
	word EnMID;
    uint8_t MotionMethod;
	word NextTop;
	word Prev1Top;
	word Prev2Top;
	word Prev3Top;
...
static const uint8_t MaxDiver[18] = { 5,8,12,15,15,15,15,15,15,15,15,15,15,15,15,15,15,15 };

static void CheckDiversants( City* CT )
{
...
static int FastCheckPosDanger( int x, int y )
{
...
static bool SearchGreAttObject( int* x, int* y, int count, uint8_t NI )
{
...
static bool FastSearchGreAttObject( int* x, int* y, int count, uint8_t NI )
{
	int rr = tmtmt % count;
	int MinTopR = 10000000;
	int xb = 0, yb = 0;
...
static AI_Army* GetNearestArmy( int x, int y, City* CT )
{
	AI_Army* AAR = CT->ARMS;
	int Abest = -1;
	int minR = 100000;
...
static bool CheckWallForKilling( OneObject* OB )
{
...
static int GetBestVictimForArchers( uint8_t NI, int x, int y, int R, int MyTop )
{
