...
// Moved from QueueManagement.cpp (author: Ereb)
/*
    Determine increment / decrement amount for production queues.
*/
static int GetQueueMultiplier()
{
...
static constexpr uint8_t MaxO = 200;
...
static constexpr int FillF_Pos = 12;
...
static MonInf MList[MaxO];
static AblInf AList[MaxO];
static uint16_t ArmList[MaxO];
static int SubIcon;
...
static void GetMonCost( byte NI, word NIndex, char* st )
{
...
static uint16_t SELSET[32];
static int NSelSet = 0;

static void HPSEL( int i )
{
...
static bool IsItInSelSet( word ID )
{
...
static void FREEORD( int /*i*/ )
{
	CmdErseBrigs( MyNation );
}

static void MSTANDGR( int /*i*/ )
{
	CmdMakeStandGround( MyNation );
}

static void CSTANDGR( int /*i*/ )
{
	CmdCancelStandGround( MyNation );
}

static bool FullBrigMode = false;

static void OLIHPRO( int /*i*/ )
{
	FullBrigMode = !FullBrigMode;
}

static void SELBRIG( int i )
{
	if (GetKeyState( VK_SHIFT ) & 0x8000)
	{
...
static void PreBrig( int i )
{
	PreviewBrig = i;
}

static void EATTACK_Pro( int /*i*/ )
{
	CmdSetSrVictim( MyNation, 0 );
}

static void DATTACK_Pro( int /*i*/ )
{
	CmdSetSrVictim( MyNation, 1 );
}

static void DOPATROL( int /*i*/ )
{
	PATROLMODE = 1;
}

static void REFORMA( int i )
{
	CmdMakeReformation( MyNation, i & 65535, i >> 16 );
}

static void FILLFORM( int i )
{
...
static void COM_DOGROUP( int /*i*/ )
{
    CmdDoGroup( MyNation );
}

static void COM_UNGROUP( int /*i*/ )
{
	CmdUnGroup( MyNation );
}
...
static const char * ARTCAPS[8] = { NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL };
...
static void OrderMove( int x, int y )
{
	CmdSendToXY( MyNation, x, y, 512 );
}

static void OrderPatrol( int x, int y )
{
	CmdPatrol( MyNation, x, y );
}

static void OrderRepair( int /*x*/, int /*y*/ )
{
	CmdSetRprState( MyNation, 1 );
}

static void OrderNucAtt( int x, int y )
{
	CmdNucAtt( MyNation, x, y );
}

static void OrderSetDst( int x, int y )
{
	CmdSetDst( MyNation, x, y );
}

static void UNIPARAM( int i )
{
	curptr = 2;
	curdx = 16;
	curdy = 15;
...
//-------------------MESSAGES-------------------//
static lpCHAR* GMessIDS = nullptr;
static lpCHAR* GMessage = nullptr;

static int NMess = 0;
static int MaxMess = 0;
