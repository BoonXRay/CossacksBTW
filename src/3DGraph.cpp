...
static int GetMax(int z1, int z2, int z3)
{
	if (z1 > z2) {
		if (z3 > z1)return z3;
		else return z1;
...
static int GetMin(int z1, int z2, int z3)
{
	if (z1 < z2) {
		if (z3 < z1)return z3;
		else return z1;
...
static void ShowClippedTriangle(int x, int y, uint8_t * data) {
	int dx = ((int*)data)[1];
	int dy = ((int*)data)[2];
    int y1 = y + dy;
	int dofs = int(data);
...
static bool PrecPrepareToRender(int NLines, int startX, int startY) {
	// BoonXRay 05.08.2017
	//__asm 
	{
		//mov		ax, VertBuf[0]
...
static bool AbsolutePrecPrepareToRender(int NLines, int startX, int startY) {
	// BoonXRay 05.08.2017
	//__asm 
	{
		//mov		ax, VertBuf[0]
...
static int PrecRenderTriangle64Dithering(int NLines, uint8_t * Dest, uint8_t * Bitmap) {
	((int*)Dest)[0] = NLines;
	int Startscan;
	int ScanSize;
	int VertPos;
...
static int PrecPreRenderTri64(int x1, int y1,
	int x2, int y2,
	int x3, int y3,
	int bmx, int bmy,
...
static int RenderBestTriangle64(int xs1, int ys1,
	int xs2, int ys2,
	int xs3, int ys3,
	int xb1, int yb1,
	int xb2, int yb2,
...
static int DirectRenderTriangle64Dithering(int NLines, int StartLine, int EndLine, int DestSizeX, uint8_t * Dest, uint8_t * Bitmap) {
...
static int DirectPreRenderTri64(int x1, int y1,
	int x2, int y2,
	int x3, int y3,
	int bmx, int bmy,
	int bmdxx, int bmdyx, int bmdxy, int bmdyy,
...
uint8_t * TexMap;//Map of textures in vertices
uint8_t * SectMap;//Map of sections on lines
...
uint16_t TexFlags[ 256 ];
uint8_t TexMedia[ 256 ];
uint8_t ExtTex[ 256 ][ 4 ];
short randoma[ 8192 ];
...
static const int HardLight[32] = { 0,1,1,3,5,7,9,11,12,12,12,12,12,12,12,12,12,12,12,12,12,12,13,13,13,13,13,13,13,13,13,13 };

int GetLighting(int i)
{
    uint8_t tex = GetVTex(i);
    uint16_t tf = TexFlags[tex];
...
void Loadtextures()
{
	...
	RSeek(f1, 0x436);
    tex1 = new uint8_t[ RFileSize(f1) - 0x436 ];
    ...
    memset( TexMedia, 0, sizeof( TexMedia ) );
...
static int RenderSector(int i, bool Mode3D, uint8_t * DST, bool NeedRender)
{
	if (i >= MaxTH*MaxTH * 2)
	{
		return 0;
...
static bool CheckPointInside(int x, int y, short* xi, short* yi, int Np) {
	int NCross = 0;
	for (int i = 1; i < Np; i++) {
		int x0 = xi[i - 1];
		int x1 = xi[i];
