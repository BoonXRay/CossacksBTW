...
static void RegisterSprite( int N )
{
...
static bool CheckTPointInside( int x, int y, T_Curve* TC )
{
	int ncr = 0;
...
static bool AddLockPts( int x0, int y0, int NPT, short* xi, short* yi, uint8_t add )
{
	T_Curve TC;
	TC.x0 = x0;
	TC.y0 = y0;
...
static void addLogTree( int x, int y )
{
	switch (InteriorType)
	{
...
static bool FindPlaceForPeasant( int* px, int* py,
	int tx, int ty, int r )
{
	int pix = ( ( *px ) ) >> 8;
	int piy = ( ( *py ) ) >> 8;
...
static void FindResInCell( CellSearch* CSR, int cell, int Top )
{
...
static bool CheckSpritesInCell( int cell, int x, int y, int r )
{
...
static bool CheckSpritesInCellNew( int cell, int x, int y, int r )
{
	if (cell < 0 || cell >= VAL_SPRSIZE)return true;
	int* CEL = SpRefs[cell];
	int   NCEL = NSpri[cell];
...
static int CheckSpritesInCellForMine( int cell, int* xi, int* yi, int /*r*/, byte rmask )
{
	if (cell < 0 || cell >= VAL_SPRSIZE)return true;
	int x = *xi;
	int y = *yi;
...
static inline int GetCell( int xs, int ys )
{
...
//RType-resource type
//-----Notes-------
static int FindResourceObject( int* xd, int* yd, int xp, int yp, byte RType )
{
	CellSearch CSR1;
	CSR1.xd = *xd;
	CSR1.yd = *yd;
...
static void FindLimResInCell( CellSearch* CSR, int cell, word Lim, int Top )
{
	if (cell < 0 || cell >= VAL_SPRSIZE)return;
	int* CEL = SpRefs[cell];
	int   NCEL = NSpri[cell];
...
static int FindLimResourceObject( int* xd, int* yd, int xp, int yp, byte RType, word Lim )
{
...
int OneSprite::PerformWork(uint16_t Effect )
{
	ObjCharacter* OCR = OC;
	WorkOver++;
...
static uint8_t FindAnyResInCell( int x, int y, int cell, int* Dist, uint8_t Res )
{
