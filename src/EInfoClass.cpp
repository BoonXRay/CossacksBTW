...
static constexpr int MAXPLAY = 7;
...
uint16_t EnemyInfo::ResearchCellForSupermortira(int cell, int MinDistance, int MaxDistance) {
	word ct = word(tmtmt);
	MinDistance <<= 4;
	MaxDistance <<= 4;
...
uint16_t EnemyInfo::SearchDangerousPlaceForEnemy(int* utx, int* uty, int MINR, int MAXR) {
...
uint16_t EnemyInfo::SearchBestEnemyAndPlaceForSupermortira(OneObject* OB, int* BestX, int* BestY) {
	int cx = OB->RealX >> 11;
	int cy = OB->RealY >> 11;
	...
	//Can I shot now?
	if (SAFETY) {
        uint16_t MID = ResearchCellForSupermortira(cell, MINR, MAXR - 64);
		if (MID != 0xFFFF) {
			*BestX = OB->RealX;
			*BestY = OB->RealY;
			return MID;
...
void EnemyInfo::AddSafePoint(int x, int y, uint16_t Index, uint16_t SN, uint16_t Prio) {
...
void EnemyInfo::ALLOCATE()
{
    ...
    InflMap = new uint32_t[TopLx*TopLx];
