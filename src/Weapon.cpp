...
static constexpr uint8_t WEPSH = 14;
...
static bool Create3DAnmObjectEX( Weapon* Weap, int xs, int ys, int zs1,
	int xd, int yd, int zd,
	OneObject* OB, byte AttType, word DestObj,
	int vx, int vy, int vz );

static void ExplodeAnmObject( AnmObject* AO )
{
	Weapon* Weap = AO->Weap;
	if ( AO->Damage )
	{
...
bool Create3DAnmObject(Weapon* Weap, int xs, int ys, int zs1,
    int xd, int yd, int zd,
    OneObject* OB, uint8_t AttType, uint16_t DestObj )
{
...
static bool CheckWpn( Weapon* WP )
{
...
static Weapon* ReturnFlyChild( Weapon* Weap )
{
	if ( CheckWpn( Weap ) )
	{
		return Weap;
...
static int CheckWpPoint( int x, int y, int z, word Index )
{
...
static int CheckWpLine( int xs, int ys, int zs, int xd, int yd, int zd, word Index )
{
