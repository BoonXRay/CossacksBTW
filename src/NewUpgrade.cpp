...
static int GetNTUnit( Nation* NT, char* Name )
{
	int nn = NT->NMon;
...
static int GetNTUpgrade( Nation* NT, char* Name )
{
	int nn = NT->NUpgrades;
	for (int i = 0; i < nn; i++)
	{
...
static int GetNTElement( Nation* NT, char* Name, bool UseUnits, bool UseUpgrades )
{
	int zz = -1;
...
//4-  0..31
static void GetWeaponKind( GFILE* f, NewUpgrade* NU, Nation* /*NT*/, char* Name, int Line )
{
	char gx[128];
	char gy[128];
	int z = Gscanf( f, "%s", gx );
...
static void GetAttIndex( GFILE* f, NewUpgrade* NU, Nation* /*NT*/, char* Name, int Line )
{
	char gx[128];
	int z = Gscanf( f, "%s", gx );
	int p1, p2;
...
static void GetCtgParam01( GFILE* f, NewUpgrade* NU, Nation* NT, char* Name, int Line )
{
	char gx[128];
	char gy[128];
    /*int z =*/ Gscanf( f, "%s", gx );
...
static void GetUnits( GFILE* f, NewUpgrade* NU, Nation* NT, char* Name, int Line )
{
	char gx[128];
	char gy[128];
	int z = Gscanf( f, "%s", gx );
...
static void GetUnitsAndUpgrades( GFILE* f, NewUpgrade* NU, Nation* NT, char* Name, int Line, bool UseUnits, bool UseUpgrades )
{
	NU->CtgType = 0;
	char gx[128];
	char gy[128];
...
static bool ReadValue( GFILE* f, int* result, int* ValType )
{
	char gx[128];
...
static void GetValue( GFILE* f, NewUpgrade* NU, Nation* /*NT*/, char* Name, int Line )
{
	int value, ValType;
	char gy[128];
...
static void GetCostValue( GFILE* f, NewUpgrade* NU, Nation* /*NT*/, char* Name, int Line )
{
	char gx[128];
	char gy[128];
	int p1, p2;
...
static void GetUpgradeCost( GFILE* f, NewUpgrade* NU, Nation* /*NT*/, char* Name, int Line )
{
	char gx[128];
	char gy[128];
	int p1, p2, p3;
...
static void GetUpgradeTime( GFILE* f, NewUpgrade* NU, Nation* /*NT*/, char* Name, int Line )
{
	char gx[128];
	char gy[128];
	int p1;
...
static void GetWeapon( GFILE* f, NewUpgrade* NU, Nation* /*NT*/, char* Name, int Line )
{
	char gx[128];
	char gy[128];
	char gz[128];
...
bool UnderstandUpgrade( GFILE* f, char* UpgName, Nation* NT, char* name, int* lpLine, uint8_t NID )
{
    ...
    const char* UpgMess = GetTextByID( gy );
...
static void UseValue( int *Value, byte Type, int NewValue )
{
	switch (Type)
	{
	case 0://XX
...
static void UpgradeSpeed( GeneralObject* GO, word /*CtgValue*/, byte ValueType, int Value, OneObject* /*OB*/ )
{
...
static void UpgradeShield( GeneralObject* GO, word /*CtgValue*/, byte ValueType, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
	int val = AC->Shield;
	UseValue( &val, ValueType, Value );
...
static void UpgradeProtection( GeneralObject* GO, word CtgValue, byte ValueType, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
...
static void UpgradeAttPause( GeneralObject* GO, word CtgValue, byte ValueType, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
...
static void UpgradeWeapon( GeneralObject* GO, word /*CtgValue*/, byte /*ValueType*/, int /*Value*/, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
...
static void UpgradeBuild( GeneralObject* GO, word /*CtgValue*/, byte ValueType, int Value, OneObject* /*param_object*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
...
static void UpgradeLife( GeneralObject* GO, word /*CtgValue*/, byte ValueType, int Value, OneObject* /*param_object*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
	int val = AC->Life;
	UseValue( &val, ValueType, Value );
...
static void UpgradeAttRange( GeneralObject* GO, word CtgValue, byte ValueType, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
...
static void UpgradeInside( GeneralObject* GO, word /*CtgValue*/, byte ValueType, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
...
static void UpgradeCost( GeneralObject* GO, word /*CtgValue*/, byte /*ValueType*/, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
	byte VT = ( Value >> 26 ) & 7;
...
static void UpgradeEnable( GeneralObject* GO, word /*CtgValue*/, byte /*ValueType*/, int /*Value*/, OneObject* /*OB*/ )
{
	GO->Enabled = true;
	GO->CondEnabled = true;
	for (int i = 0; i < 8; i++)
...
static void UpgradeDamage( GeneralObject* GO, word CtgValue, byte ValueType, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
...
static void UpgradeGetRes( GeneralObject* GO, word /*CtgValue*/, byte ValueType, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
	int val = AC->ResEff;
	UseValue( &val, ValueType, Value );
...
static void ApplyToOneUnitAndOneCategory( GeneralObject* GO, word Category, NewUpgrade* NU, UpgradeFN* UFN, OneObject* OB )
{
	if (NU->ValGroup)
	{
...
static void UpgradeSingleAttPause( GeneralObject* /*GO*/, word /*CtgValue*/, byte ValueType, int Value, OneObject* OB )
{
	if (!OB)return;
...
static void UpgradeSingleInside( GeneralObject* /*GO*/, word /*CtgValue*/, byte ValueType, int Value, OneObject* OB )
{
	if (!OB)return;
...
static void UpgradeFishing( GeneralObject* GO, word /*CtgValue*/, byte ValueType, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
...
static void UpgradeRazbros( GeneralObject* GO, word /*CtgValue*/, byte ValueType, int Value, OneObject* /*OB*/ )
{
	AdvCharacter* AC = GO->MoreCharacter;
	int val = AC->Razbros;
	UseValue( &val, ValueType, Value );
...
static void UpgradeShar( GeneralObject* /*GO*/, word /*CtgValue*/, byte /*ValueType*/, int /*Value*/, OneObject* OB )
{
...
static void HealUnits( GeneralObject* GO, word /*CtgValue*/, byte /*ValueType*/, int /*Value*/, OneObject* OB )
{
...
static void ApplyToOneUnit( GeneralObject* GO, NewUpgrade* NU, UpgradeFN* UFN, OneObject* OB )
{
...
static void ApplyToUnits( Nation* NT, NewUpgrade* NU, UpgradeFN* UFN, OneObject* OB )
{
	if (NU->UnitGroup)
	{
		if (NU->UnitType == 1)
