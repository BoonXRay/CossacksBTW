...
static void ChatBroadcastKeyChanged( CHAT /*chat*/,
    const char * /*channel*/,
    const char * /*user*/,
    const char * /*key*/,
    const char * /*value*/,
...
bool ChatSystem::ConnectToChat( const char* Nick, const char* Info, const char* Mail, const char* Chat )
{
...
void ChatSystem::SortPlayers( OneChatPlayer* PL, int N )
{
	...
	do
	{
		...
		for (int i = 1; i < N; i++)
		{
			...
			if (r < 0)
			{
                uint8_t PLX[sizeof( OneChatPlayer)];
