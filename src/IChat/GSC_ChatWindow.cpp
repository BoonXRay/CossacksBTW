...
static CustomBorder* CUBM;
static CustomBorder* CUBC;
static bool CM_Vis = 0;
static bool CC_Vis = 0;
static byte RCOLOR = 0;

static void Draw_PLIST( int x, int y, int /*Lx*/, int /*Ly*/, int Index, byte Active, int /*param*/ )
{
	if (!RCOLOR)
	{
...
static void EnterPersonalMessage( char* Nick, bool Active )
{
	char nick[128];
...
static void ShowCentralMessage2C( const char* Message, int GPIDX )
{
...
static bool NeedSmallSize = 0;
static bool NeedBigSize = 0;
static bool LockResize = 0;
...
static bool RunHomePageAnyway = 0;
...
int Process_GSC_ChatWindow( bool Active, RoomInfo* RIF )
{
	...
	if (Active && !CSYS.Connected)
	{
        ...
        const char * Nick = SXP_GetVar( 2, "%NICK" );
        const char * Prof = SXP_GetVar( 2, "%PROF" );
        const char * Mail = SXP_GetVar( 2, "%MAIL" );
		if (!Nick)
			Nick = "UNKNOWN";
		if (!Prof)
			Prof = "";
	...
	if (Active)
	{
		...
		{
            RunSXP( 0, "LF|LW_new|open&req.txt",
				INI.GetInt( "EXPLORER_X" ) + menu_x_off,
				INI.GetInt( "EXPLORER_Y" ) + menu_y_off,
				INI.GetInt( "EXPLORER_X1" ) + menu_x_off,
				INI.GetInt( "EXPLORER_Y1" ) + menu_y_off );
	...
	do
	{
        ...
        SXP_SetVar( 0, "%CP_NICK", CURRPL );
        SXP_SetVar( 0, "%CP_PROF", CURRPL );
        SXP_SetVar( 0, "%CP_MAIL", CURRPL );
        SXP_SetVar( 0, "%CV_LOBBY", LobbyVersion );

		char vv[32];

		sprintf( vv, "%d", dwVersion );

        SXP_SetVar( 0, "%CV_ENGINE", vv );
        ...
        char* VAR = SXP_GetVar( 0, "%COMMAND" );
		if (VAR)
		{
			if (!strcmp( VAR, "CGAME" ))
			{
                char* GID = SXP_GetVar( 0, "%CG_GAMEID" );
				if (GID)
				{
                    char* GAMENAME = SXP_GetVar( 0, "%CG_GAMENAME" );
					if (GAMENAME)
					{
                        char* MaxPL = SXP_GetVar( 0, "%CG_MAXPL" );
						if (MaxPL)
						{
							int mp = atoi( MaxPL );
							if (mp < 2)mp = 2;
							...
							VAR[0] = 0;
                            char* PlName_local = SXP_GetVar( 0, "%CG_PLNAME" );
							if (PlName_local)
							{
							...
							//Player's ID
                            const char *player_id_str = SXP_GetVar( 0, "%PROF" );
							if (player_id_str)
							{
								char *end;
							...
							//Server port
                            const char *hole_port_str = SXP_GetVar( 0, "%CG_HOLEPORT" );
							if (hole_port_str)
							{
								char *end;
							...
							//Server ip
                            const char *hole_host_str = SXP_GetVar( 0, "%CG_HOLEHOST" );
							if (hole_host_str)
							{
                            ...
                            const char *hole_interval_str = SXP_GetVar( 0, "%CG_HOLEINT" );
				...
				if (!strcmp( VAR, "JGAME" ))
				{
					...
					if (GID)
					{
						...
						if (GAMENAME)
						{
							...
							if (MaxPL)
							{
                                char* GAMEIP = SXP_GetVar( 0, "%CG_IP" );
								if (GAMEIP)
								{
                                    ...
                                    const char *port_str = SXP_GetVar( 0, "%CG_PORT" );
									if (port_str)
									{
										char *end;
