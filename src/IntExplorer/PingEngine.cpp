...
static int pcallT=0;
static bool decode_resp(char *buf, int bytes,struct sockaddr_in *from,int* time);
...
void PingEngine::Process()
{
	...
	sockaddr_in from;
	int L=sizeof from;
    int bread = recvfrom(sockRaw,recvbuf,MAX_PACKET,0,(sockaddr*)&from,&L);
...
static PingEngine PE;

int GETPING( uint32_t IP)
{
