#ifndef INTEXPLORER_H
#define INTEXPLORER_H
...
class OneSicWindow;
typedef uint32_t fnSendRequest(char* request, int size);
typedef uint32_t fnGetRequestResult(uint32_t Handle, char** Result, int* Size);
typedef void fnCloseRequest(uint32_t Handle);
...
public:
    ...
    RLCFont* GetFontByName(const char *fnt);
    ...
    void SendTableRefresh(char* Name, const char *server);
...
public:
	...
	void Process();
	void Erase();
    char* CreateVar(const char *Name, int Len);
