...
static int CReadX( char** sx, int x0, int x1 )
{
	int xx = 0;
	int com = 0;
	char* s = *sx;
...
static int CReadY( char** sx, int x0, int x1, OneBox* BOX )
{
	int xx = 0;
	int com = 0;
	char* s = *sx;
...
static void ReadCoordinates( char** src, int& x, int& y, int& xR, int& yR, OneBox* BOX )
{
	char c;
	int x0 = 0;
	int y0 = 0;
...
RLCFont* sicExplorer::GetFontByName( const char* fnt )
{
...
char* OneSicWindow::CreateVar( const char* Name, int Len )
{
	char* GV = GetVar( Name );
	if (GV)
		return GV;
...
char* OneSicWindow::GetVar( const char* Name )
{
...
void sicExplorer::SendTableRefresh( char* Name, const char* server )
{
