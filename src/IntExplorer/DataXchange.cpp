...
//bool GameInProgress;
static bool LOGMODE = 0;
static void ReportIt( const char* s, ... )
{
	if (!LOGMODE)return;
	SYSTEMTIME ST;
	GetSystemTime( &ST );
...
public:
    ...
    DWORD SendRequest(sicExplorer* SXP, const char *request, bool AllowNewWindow, bool Auto );
	...
	void  CloseAll();
	void  Shutdown();
    void  RegisterDevice(const char *ID, const char *DLL_Path );
};
...
static void FilterRQ2Send( sicExplorer* SXP, ParsedRQ* RQ, bool AllowNew );

DWORD SXP_DevScope::SendRequest( sicExplorer* SXP, const char* request, bool AllowNewWindow, bool Auto )
{
	ParsedRQ P1;
	P1.Parse( request );
	if (LOGMODE)
...
void SXP_DevScope::RegisterDevice( const char* ID, const char* DLL_Path )
{
...
static int LoadDefaults( byte** Data )
{
...
static char* SaveCurrentResult( OneSicWindow* OSW, char* dest )
{
	if (OSW->Result)
	{
        sprintf( dest, "Internet/Cash/tempcml_%X.cml", uint32_t( GetTickCount() ) );
...
static const char* SaveCurrentResultTo( OneSicWindow* OSW, const char* dest )
{
	if (OSW->Result)
	{
		ResFile F = RRewrite( dest );
...
static void exec_LW_time( int Np, char** par, int* /*size*/ )
{
	if (Np < 2)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_move( int Np, char** par, int* /*size*/ )
{
	if (Np < 3)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_lockall( int Np, char** par, int* /*size*/ )
{
	SetCurPtr( 14 );
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_lockbox( int Np, char** par, int* /*size*/ )
{
	if (Np < 2)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_lock( int Np, char** par, int* /*size*/ )
{
	if (Np < 2)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_visbox( int Np, char** par, int* /*size*/ )
{
	if (Np < 2)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_vis( int Np, char** par, int* /*size*/ )
{
	if (Np < 2)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_enbbox( int Np, char** par, int* /*size*/ )
{
	if (Np < 2)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_enb( int Np, char** par, int* /*size*/ )
{
	if (Np < 2)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_mapdata( int Np, char** par, int* /*size*/ )
{
	char* win = par[Np - 1];
	char widx = win[0];
	if (widx >= '0'&&widx <= '7')
...
static void exec_LW_show( int Np, char** par )
{
	SetCurPtr( 0 );
	if (Np < 2)return;
	char* win = par[Np - 1];
...
static void exec_LW_file( int Np, char** par )
{
	SetCurPtr( 0 );
	if (Np < 2)return;
	char* win = par[Np - 1];
...
static void exec_LW_pfile( int Np, char** par )
{
	SetCurPtr( 0 );
	if (Np < 2)return;
	char* win = par[Np - 1];
...
static void exec_LW_cfile( int Np, char** par, int* size )
{
	if (Np < 1)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_bonus( int Np, char** par, int* size )
{
	if (Np < 1)return;
	char MON[32];
...
static void exec_LW_message( int Np, char** par, int* size )
{
	if (Np < 1)return;
	char VV[1024];
	VV[0] = 0;
...
static void exec_LW_abort( int /*Np*/, char** /*par*/, int* /*size*/ )
{
	ExitProcess( 0 );
}

static void exec_LW_mupdate( int Np, char** par, int* /*size*/ )
{
	if (Np < 1)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static ResFile ForceOpenFile( char* Name )
{
	char tmp[512];
	char tmp1[512];
	int pos = 0;
...
static void exec_LW_mpt( int Np, char** par, int* size )
{
	if (Np < 4)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_mpclear( int /*Np*/, char** /*par*/, int* /*size*/ )
{
...
static void exec_LW_xcfile( int Np, char** par, int* size )
{
	if (Np < 3)
	{
		return;
		...
		for (int q = 0; q < NW; q++)
		{
			...
			if (!strcmp( OSW->WinID, win ))
			{
				...
				if (F != INVALID_HANDLE_VALUE)
				{
					...
					_utimbuf UTB;
					UTB.actime = 0;
                    sscanf( par[1], "%ld", &UTB.actime );
                    sscanf( par[1], "%ld", &UTB.modtime );
					char tmp[512];
...
static void exec_LW_key( int Np, char** par, int* /*size*/ )
{
	if (Np < 2)return;
	char* win = par[1];
	char widx = win[0];
...
static void exec_LW_gvar( int Np, char** par, int* /*size*/ )
{
	if (Np < 2)return;
	char* win = par[Np - 1];
	char widx = win[0];
...
static void exec_LW_compose( int Np, char** par, int* /*size*/ )
{
	if (Np < 3)return;
	char fnm[256];
...
static void exec_LW_proc( int Np, char** par, int* /*size*/ )
{
...
static void exec_LW_dtbl( int Np, char** par, int* size )
{
	char* win = par[Np - 1];
	char widx = win[0];
	if (widx >= '0'&&widx <= '7')
...
static void exec_LW_tbl( int Np, char** par, int* /*size*/ )
{
	char* win = par[Np - 1];
	char widx = win[0];
	if (widx >= '0'&&widx <= '7')
...
static void exec_LW_dfp( int Np, char** par, int* size );
static void exec_LW_dff( int Np, char** par, int* size );
static void exec_LW_uprec( int Np, char** par, int* size );
static void exec_LW_showrec( int Np, char** par, int* size );
...
static OneDnProcess* DNPR = nullptr;
static int N_DNPR = 0;
static int Max_DNPR = 0;
...
static char CurrUplID[128] = "";
static int CurrUplPos = 0;
