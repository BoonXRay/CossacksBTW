...
static bool AddArea(short x, short y, byte Sliv)
{
	if (NAreas > 1016)
	{
		return false;
    ...
    // BoonXRay 10.09.2017 This return value not checked
    return true;
}
...
static void AddChTop(uint16_t TopID)
{
...
static void AddSurroundingAreas(uint16_t TopID)
{
	if (!NAreas)return;
	AddChTop(TopID);
...
static void LimitedSetNewLinks(word* AList, int Na, word* MLinks, word* MDist)
{
...
static void OnePartIterationNew(int Nitr);
static void RemakeGroupOfAreas(word* Grp, int Na);
int NeedITR = 100;
...
int   WNAreas;
int WMaxArea;
uint16_t * WMotionLinks;
uint16_t * WLinksDist;

#define MinWNorm (8<<(ADDSH-1))
static bool AddWArea(short x, short y)
{
	for (int i = 0; i < WNAreas; i++) {
...
static void AddWLink(int N1, int N2)
{
	Area* AR = &WTopMap[N1];
	int N = AR->NLinks;
	for (int i = 0; i < N; i++) {
...
static int SearchID(word* Set, int N, word ID)
{
	for (int i = 0; i < N; i++)
	{
		if (Set[i] == ID)
...
static void EraseAllLinks(int Strat, int Fin);
...
static OLDLINK* OLN = NULL;

static void SymmetrizeLinks()
{
	for (int i = 0; i < NAreas; i++) {
		Area* AR = TopMap + i;
...
static void EraseLinks(int X, int A)
{
	int N = OLN->NLinks[A];
...
static void EraseAllLinks(int Start, int Fin)
{
	int NS = Start*NAreas;
