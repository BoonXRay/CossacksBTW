...
static bool CorrectOnePosition( int* x, int* y )
{
	int xx = *x;
	int yy = *y;
...
struct Brigade_LST :public BrigadeOrder
{
	int time;
	int N;
	int ZastTime;
...
static const char* LS_Mess = "[Local sent to]";
...
static const char* CM_Mess = "[Capture mine]";
...
static const char* BTL_Mess = "[Make Battle]";
...
static const char* HLST_Message = "[HumanLocalSendTo]";
...
static const char* KP_Message = "[KeepPosition]";
...
static bool CheckBDirectWay( int x0, int y0, int x1, int y1 )
{
	int bx = x0 << 10;
	int by = y0 << 10;
	int Lx = x1 - x0;
...
static bool CalcCenter( Brigade* BR, int* x, int* y )
{
...
static const char* HGST_Message = "[HumanGlobalSendTo]";
...
//------------------------------Bitva------------------------
static const char* BBIT_Message = "[Brigade::Bitva]";
//#define maxen
...
void CorrectImSelectionInGroups( uint8_t NI )
{
    word FormsIDS[256];
    int NForms = 0;
    int N = ImNSL[NI];
...
//==========================RESEARCH ISLANDS===============================//
int NIslands = 0;
static constexpr int MaxIsl = 64;
static int IslandX[ MaxIsl ];
static int IslandY[ MaxIsl ];
static uint8_t IslPrs[ MaxIsl ];

uint16_t * TopIslands = NULL;
static uint16_t * NearWater = NULL;

static void ArrangeAreas( int ID, byte IsID, int Deep )
{
	TopIslands[ID] = IsID;
	if ( Deep < 100 )
	{
...
static void ProduceBattleShip( byte nat, word ID )
{
	int N = NtNUnits[nat];
	word* Uni = NatList[nat];
	for ( int i = 0; i < N; i++ )
...
static void ProduceLodka( byte nat, word ID )
{
	int N = NtNUnits[nat];
	word* Uni = NatList[nat];
	for ( int i = 0; i < N; i++ )
...
static uint16_t SearchMineToDestroy( Brigade* BR )
{
	int NGalley = 0;
    uint16_t GalleyID[32];
	int GalleyX[32];
	int GalleyY[32];
	int GalleyR = 0;
...
static uint16_t SearchEnemyToDestroy( Brigade* BR )
{
	int RMIN[128];
	int RMAX[128];
    //int DANG[128];
...
static int TryToBeatByShips( Brigade* BR )
{
...
static void LocalSendShipsOld( Brigade* BR, int x, int y, byte Type )
{
...
static inline void SetPX( int x, int y, byte Val )
{
	if ( x >= 0 && y >= 0 && x < MTRLX&&y < MTRLY )MTRPTR[x + y*MTRLX] = Val;
}

static void LocalSendShips( Brigade* BR, int x, int y, byte Type )
{
...
static word GetNearestBrig( City* CT, word Top )
{
	int D = 1000000;
	int BRB = -1;
	int TP1 = Top*WNAreas;
...
static word GetNearestBrigToKillUnitsAndTowers( City* CT, word Top )
{
	int D = 1000000;
	int BRB = -1;
	int TP1 = Top*WNAreas;
...
static word GetNearestBrigWithType( City* CT, word Top, byte Type )
{
	int D = 1000000;
	int BRB = -1;
	int TP1 = Top*WNAreas;
...
static word DetermineWaterTopology( Brigade* BR )
{
	int NTops = 0;
	//int NInTop[64];
	int Tops[128];
...
static int GetWarInCell( int cell )
{
	cell += VAL_MAXCX + 1;
	int NMon = MCount[cell];
    if ( !NMon )return 0;
...
static bool CheckUnitsNearPoint( int x, int y )
{
...
static int GetTrInCell( int cell )
{
	cell += VAL_MAXCX + 1;
	int NMon = MCount[cell];
	if ( !NMon )
...
static word CheckTransportNearPoint( int x, int y )
{
	int cell = ( ( y >> 3 ) << VAL_SHFCX ) + ( x >> 3 );
	int rx1 = 2;
	int rx2 = rx1 + rx1 + 1;
...
static int FindCostPoint( int x, int y, byte Isl )
{
	if ( !NCost )return -1;
	int MinR = 100000;
	int BestPL = -1;
...
static int FindCostPointEx( int x, int y, byte Mask )
{
	if ( !NCost )return -1;
	int MinR = 100000;
	int BestPL = -1;
