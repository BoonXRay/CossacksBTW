...
public:
	...
	word* CellPretty;
	*/
    uint16_t FindNextCell(int F, int Start, NetSample* Net);
    uint16_t FindWayTo(int F, int Start, int Fin, NetSample* Net);
	//void CreateMaps();
	SafeNet();
	~SafeNet();
...
void NetSample::CreateDiversantMap(byte NI)
{
	...
	if (NZ != NAreas) {
        Danger = (uint16_t *)realloc(Danger, 2 * NAreas);
        Pretty = (uint16_t *)realloc(Pretty, 2 * NAreas);
		NZ = NAreas;
    }
	int N2 = 2 * NAreas;
    uint16_t addDam[2048];
	memset(Danger, 0, N2);
	memset(Pretty, 0, N2);
	memset(addDam, 0, N2);
	byte Mask = 1 << NI;
...
static constexpr uint16_t MAXAR = 700;

uint16_t SafeNet::FindNextCell(int F, int Cell, NetSample* Net)
{
	int NZ = Net->NZ;
	if (Cell < NZ) {
        int F1 = F >> 1;
        uint16_t * CellDanger = Net->Danger;
        uint16_t * CellPretty = Net->Pretty;
        uint16_t AddPrett[2048];
        ...
        uint16_t DistArr[2048];
        uint16_t DistID[2048];
		byte Checked[2048];

		memset(DistArr, 0, NAreas << 1);
		memset(Checked, 0, NAreas);
        uint16_t BoundCells[200];
		int NBound = 0;
        uint16_t NewBound[200];
		int NNewB = 0;

		NBound = 1;
		BoundCells[0] = Cell;
		DistArr[Cell] = 1;
        uint16_t LastPrettyID = 0xFFFF;
		do {
			NNewB = 0;
...
uint16_t SafeNet::FindWayTo(int F, int Cell, int Fin, NetSample* Net) {
	//CreateMaps();
	if (Cell < Net->NZ) {
        uint16_t * CellDanger = Net->Danger;
        int F1 = F >> 1;
