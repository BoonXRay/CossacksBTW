...
static int GetWTP1( int x, int y );
...
static uint8_t GD8[9] = { 7,6,5,0,0,4,1,2,3 };
static char DX8[8] = { 0,1,1,1,0,-1,-1,-1 };
static char DY8[8] = { -1,-1,0,1,1,1,0,-1 };

static uint8_t GetDir8( int dx, int dy )
{
...
int WallCluster::CreateData(uint16_t *Data, uint16_t Health )
{
...
void WallCluster::SendSelectedToWork(uint8_t pNI, uint8_t OrdType )
{
	int NSel = NSL[pNI];
	word* SMon = Selm[pNI];
    int CurCell = 0;
...
void WallCluster::CreateByData(uint16_t *Data )
{
	NI = ( (byte*) Data )[2];
	Type = ( (byte*) Data )[3];
	NCells = Data[2];
...
static const char* CluF = "      "
" **** "
" **** "
" **** "
" **** "
"      ";
static const char* GetLockData( byte id )
{
	switch ( id )
	{
	case 0:return CluF;
...
bool WallCell::CheckPosition()
{
	...
	int y0 = ( y << 2 ) - 1;
	int Nx = 0;
    const char* Linf = GetLockData( Sprite );
	int Npt = 0;
	int NLock = 0;
	for ( int i = 0; i < 36; i++ )
	{
...
int WallCell::GetLockStatus()
{
	...
	int y0 = ( y << 2 ) - 1;
	int Nx = 0;
    const char* Linf = GetLockData( Sprite & 15 );
	int LData = 0;
	int LDMask = 1;
	for ( int i = 0; i < 36; i++ )
	{
...
static void WallBuildStage( WallCell* WC )
{
...
static void SetCellLife( int x, int y, int Life, bool Base )
{
	int LI = GetLI( x, y );
	if ( LI >= 0 && LI < MaxLI )
	{
...
static const int  Wdx[4] = { 0,1,1,1 };
static const int  Wdy[4] = { -1,-1,0,1 };
...
static void LoadWallSprites( char* Name, char* Gate, word Type )
{
...
WallSystem WSys;
static uint8_t WallType;

bool BuildWall;
static bool FirstWall;
...
static int GetTopDistanceW( int xa, int ya, int xb, int yb )
{
...
//Building the walls
static bool GetFreeCell( int* x, int* y, int bx, int by, int *bdist )
{
	int x1 = ( *x << 2 ) + 1;
	int y1 = ( *y << 2 ) + 1;
	int xx = ( *x ) + 1;
...
static bool FindPlaceForWallBuilder( byte NI, int *x, int *y, int bx, int by )
{
...
static bool CheckSideForBuild( byte NI, int x, int y )
{
...
static void SetWTP( int x, int y, int Sprite, int Base )
{
	int LI = GetLI( x, y );
	if ( LI >= 0 && LI < MaxLI )
	{
...
static void ControlSquish();

static void PlayOpenGate( Gate* Gat, WallCharacter* WCR )
{
...
static void PlayCloseGate( Gate* Gat, WallCharacter* WCR )
{
...
static void SquishUnitsInCell( int cell, int x, int y, int R )
{
	cell += VAL_MAXCX + 1;
...
static void SquishUnits( int x, int y, int R, byte /*NMask*/ )
{
...
static void CloseCell( int x, int y )
{
	int LI = GetLI( x, y );
	x *= 4;
	y *= 4;
