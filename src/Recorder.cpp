...
void RecordGame::AddByte(uint8_t v )
{
	if ( Size >= MaxSize )
	{
		MaxSize += 65536;
...
void RecordGame::AddBuf(uint8_t *Buf, int sz )
{
	if ( Size >= MaxSize - sz )
	{
		MaxSize += 65536;
...
void RecordGame::ReadStream( char* Name )
{
	...
	if ( STREAM.Open( Name ) )
	{
        ...
        if( ( SIGN != ConvCosChar( '[', 'C', 'E', 'R' ) ) || STREAM.Error() )
		{
            //if ( SIGN == ']CER' )
            if ( SIGN == ConvCosChar( ']', 'C', 'E', 'R' ) )
			{
				RecFormatID = 1;
			}
			else
						...
						for ( int k = 1; k < 7; k++ )
						{
							...
							{//strcmp(PINFO[k].name,NoPlayer)){
                                ...
                                for ( int w = 0; w < 10; w++ )
								{
                                    ...
                                    const char* AIN = GetTextByID( ccx );
									if ( AIN != ccx )MaxA = w;
								}
								if ( MaxA == -1 )
								{
...
static int RunSize0 = 0;
int RunDataSize = 0;
uint8_t RunData[2048];
...
static void GetFHashName( char* pass, char* Dst )
{
	int L = strlen( pass );
    unsigned int S = 0;
...
bool InternetStream::Open( char* Name )
{
	if ( Name[2] == '|' )
	{
        ...
        StartDownloadInternetFile( StreamName, Server, HashName );
		CurPosition = 0;
		LocalFile = 0;
		return true;
	}
...
static const char* DOWNLOADING = NULL;

int InternetStream::ReadSomething(uint8_t *Buf, int MaxLen, bool Scroll, bool Blocking )
{
	if ( LocalFile )
	{
